...
const LimitOrderButton = ({ disabled, token0, token1, poolAddress, wasInverted, sellPrice, tickSpacing, zeroToOne, limitOrderPlugin }: LimitOrderButtonProps) => {
    ...
    const { isLoading: isPlaceLoading } = useTransactionAwait(placeData?.hash, { type: TransactionType.LIMIT_ORDER, title: `Buy ${formatCurrency.format(Number(amount?.toSignificant()))} ${amount?.currency.symbol}` })
